...
![Abstract illustration of AI image generation systems](assets/readme-hero.png)
...
- **[Emu](https://image.tinchak0207.xyz)** – Online GPT Image 2 & Nano Banana Pro image generator. Sign in and generate — no relay station setup or API key required. OpenAI-compatible generation API and MCP integration. [Docs](https://image.tinchak0207.xyz/docs)
...
- **[MiniMax Image API](https://platform.minimax.io/docs/guides/image-generation)** – Text-to-image and image-to-image REST API from MiniMax. image-01 model supports subject reference images, configurable aspect ratios, and batch generation up to 9 images per request. [Docs](https://platform.minimax.io/docs/guides/image-generation)
- **[ModelsLab](https://modelslab.com)** – REST API for 10k+ image models including FLUX, SDXL, SD 1.5, LoRA fine-tunes, and community variants. Text-to-image, img2img, inpainting, ControlNet. Python, JS, PHP, Go, Dart SDKs. [Docs](https://docs.modelslab.com/image-generation) | SDK: Python, JS/TS, PHP, Go, Dart
- **[Novita AI](https://novita.ai)** – Serverless API with 200+ image models including FLUX.1 Kontext, Hunyuan Image 3, Seedream, and SD variants. Text-to-image, img2img, inpainting, ControlNet. Python and JS SDKs. [Docs](https://novita.ai/docs/api-reference/api-reference-overview) | SDK: [Python](https://github.com/novitalabs/python-sdk)
...
- **[Pollinations.ai](https://pollinations.ai)** – Free open-source generative AI platform. Image generation via GET endpoint (FLUX, Turbo, Kontext). Anonymous tier; register for watermark-free access and higher rate limits. [Docs](https://raw.githubusercontent.com/pollinations/pollinations/master/APIDOCS.md)
- **[Prodia](https://prodia.com)** – Serverless image generation API with 50+ models including FLUX.1/2, Recraft V4, and SD variants. 190ms latency for FLUX. Sync and async endpoints. JavaScript (prodia-js) and Python SDKs. [Docs](https://docs.prodia.com) | SDK: [JS](https://github.com/prodialabs/prodia-js), Python
...
- **[ERNIE-Image](https://github.com/baidu/ERNIE-Image)** – 8B single-stream DiT text-to-image model from Baidu. Strong text rendering for posters and infographics. Diffusers-native via ErnieImagePipeline. Turbo variant for 24GB VRAM. Apache-2.0. [Docs](https://huggingface.co/baidu/ERNIE-Image)
...
- **[SANA](https://github.com/NVlabs/Sana)** – Efficient high-resolution T2I model from NVIDIA. Linear DiT architecture with DC-AE (32x compression). Generates 4K images 20x smaller and 100x faster than Flux-12B. Diffusers-native via SanaPipeline. Apache-2.0. [Docs](https://huggingface.co/Efficient-Large-Model/SANA1.5_1.6B_1024px_diffusers)
...
- **[comfy-pack](https://github.com/bentoml/comfy-pack)** – Toolkit for locking, packaging, and deploying ComfyUI workflow environments. Bundles custom nodes, model hashes, and Python deps into a .cpack.zip. Serves workflows as REST APIs. [Docs](https://www.bentoml.com/blog/comfy-pack-serving-comfyui-workflows-as-apis)
...
- **[Fluxgym](https://github.com/cocktailpeanut/fluxgym)** – Minimal Gradio web UI for FLUX.1 LoRA training on low VRAM (12GB–20GB). Wraps Kohya scripts with automatic image resizing, HuggingFace publish, and 100% Kohya feature access via Advanced tab.
...
- **[SimpleTuner](https://github.com/bghira/SimpleTuner)** – General fine-tuning kit for diffusion models. Supports FLUX.1/2, SDXL, SD3, and more. Multi-GPU training, aspect bucketing, embedding caching, and a web UI. AGPL-3.0.
...
- **[chaiNNer](https://github.com/chaiNNer-org/chaiNNer)** – Node-based GUI for chaining image processing tasks. Supports PyTorch, NCNN, ONNX, and TensorRT upscaling models, background removal, and batch processing. Cross-platform with CUDA/ROCm/MPS backends.
- **[ComfyUI Impact Pack](https://github.com/ltdrdata/ComfyUI-Impact-Pack)** – ComfyUI custom node pack for detection-based face detailing, iterative upscaling, segmentation masking, and regional sampling. FaceDetailer node is widely used for portrait refinement.
- **[comfyui_controlnet_aux](https://github.com/Fannovel16/comfyui_controlnet_aux)** – ComfyUI custom node set for generating ControlNet hint images. Preprocessors include Canny, HED, depth estimation, pose detection, segmentation, and 20+ others. Apache-2.0.
...
- **[Cog](https://github.com/replicate/cog)** – Open-source tool for packaging ML models into Docker containers. Defines environment via cog.yaml, auto-generates a REST prediction API (Rust/Axum server), and deploys to Replicate or any Docker host. [Docs](https://cog.run)
- **[HuggingFace Inference Providers](https://huggingface.co/docs/inference-providers)** – Unified Python/JS client routing requests to fal, Replicate, Together, WaveSpeedAI, and others. Supports text-to-image tasks including FLUX models. Free tier; single HF token for all providers. [Docs](https://huggingface.co/docs/inference-providers/index) | SDK: Python (pip install huggingface_hub), JS (npm install @huggingface/inference)
...
- **[Artificial Analysis Image Leaderboard](https://artificialanalysis.ai/image/leaderboard/text-to-image)** – Elo-rating leaderboard for text-to-image models based on blind user comparisons. Covers quality, speed, and price. Separate rankings for open-weight and API-only models. [Docs](https://artificialanalysis.ai/image)
